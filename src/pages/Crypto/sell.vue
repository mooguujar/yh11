  ...
  <div class="cionBuy">
    ...
    <div class="content clearfix">
      <div class='amount'>
        ...
        <div class='top_sellNum clearfix'>
          <div class='can_sell left'>可售数量 {{ canSaleAmount.toTruncFixed(2) }}</div>
          <div class='cant_sell right'>不可售数量 {{ notSaleAmount.toTruncFixed(2) }} <van-popover v-model:show="showPopover" placement="top"
      ...
      <div class="sellNum">
        ...
        <van-field v-model="value" type="number" :placeholder="sell_coin_limit_placeholder">
          <template #button>
            <span @click="value = wallets">全部</span>
          </template>
          <template #right-icon>
            <span>GDB</span>
          </template>
        ...
        <van-radio-group v-model="checked" direction="horizontal" shape="dot">
          <div class="flex items-center">
            ...
            <van-popover placement="top-start"
                         className="sell_popover van-popup van-popover van-popover--light">
              允许多个不同买家购买此卖单，<br />并各自定义想要购买的GDB数量
              <template #reference>
                <van-icon name="question-o" />
              </template>
            </van-popover>
        ...
        <div v-if="checked == '1'">
          ...
          <van-field v-model="minimum" placeholder="0" type="number" >
          </van-field>
        </div>
        ...
        <div className="paypic">
          <van-row >
            <van-col span="7.5" class="payBox" v-if="withdrawddresslist.some(e => {
              return e.coin_protocol == 'BANK' && e.status === 1
            })
              " :class="payMateActive.includes('BANK') ? 'active' : ''" @click="payBoxTap('BANK')">
              <div>
                <van-image class="Spic credit" :src="requireImg('common/credit.png')" />
                银行卡
              </div>
              <div class='hotPic flex_s'>
                <van-image class="icon_huo " :src="requireImg('common/icon_huo.png')" />
            ...
            <van-col span="7.5" class="payBox" v-if="withdrawddresslist.some(e => {
              ...
              " :class="payMateActive.includes('ALIPAY') ? 'active' : ''" @click="payBoxTap('ALIPAY')">
              <div>
                <van-image class="Spic alipay" :src="requireImg('common/alipay.png')" />
                支付宝
              </div>
              <div class='hotPic flex_s'>
                <van-image class="icon_huo " :src="requireImg('common/icon_huo.png')" />
            ...
            <van-col span="7.5" class="payBox" v-if="withdrawddresslist.some(e => {
              ...
              " :class="payMateActive.includes('WXPAY') ? 'active' : ''" @click="payBoxTap('WXPAY')">
              <div>
                <van-image class="Spic WeChatPay" :src="requireImg('common/WeChatPay.png')" />
                微信
              </div>
              <div class='hotPic flex_s'>
                <van-image class="icon_huo " :src="requireImg('common/icon_huo.png')" />
        ...
        <van-button type="primary" class="sellButton" @click="onConfirm">
          确认出售
        </van-button>
      </div>
    </div>
    ...
    >
      ...
      <div class="confirm-dialog__content">
        <h4>{{ confirmAmount }}<span>GDB</span></h4>
        <van-divider />
        <form class="confirm-dialog__form" @submit.prevent="onSubmit">
          <div class="confirm-dialog__form-item">
            <label>支付密码</label>
            <van-field v-model="passwdValue" placeholder="请输入支付密码" />
          </div>
          <van-button type="primary" native-type="submit" block class="confirm-dialog__form-btn">
            提交
          </van-button>
        </form>
      </div>
...
const canSaleAmount = computed(() => {
  ...
  if (freeSellAmount > coinCash) return coinCash
  return freeSellAmount
})
...
const notSaleAmount = computed(() => {
  ...
  return coinCash - canSaleAmount.value
})
...
const splitMin = computed(() => {
  const sellCoinLimit = (config.value.Web_Transfer_BuySell_Coin_Fee?.Sell_Coin_Limit as any)?.v.split("-")
...
const sell_coin_limit_placeholder = computed(() => {
  const min = splitMin.value.toFixed(2)
  const max = splitMax.value.toFixed(2)
  return `限额${min}~${max}`
})
...
const onConfirm = () => {
  ...
  if (Number(value.value) < splitMin.value || Number(value.value) > splitMax.value) {
    return showToast({ message: `卖币限额\n${splitMin.value.toFixed(2)}~${splitMax.value.toFixed(2)}`, icon: infoIcon })
  }
  if (checked.value === '1') {
    if (Number(minimum.value) < splitMin.value) {
      return showToast({ message: `最低买入不能设置低于${splitMin.value.toFixed(2)}`, icon: infoIcon })
    }
  ...
  if (passwdValue.value.length == 6) {
    ...
    await goSell()
      .then(order_id => {
        ...
        passwdValue.value = ''
        showKeyboard.value = false
        router.push('/orderCreate?order_id=' + order_id + '&sell=true&payMateActive='+payMateActive.value.join(','))
        // goBuySellCoinInfoApi();
      })
      .catch(err => {
        passwdValue.value = ''
