<template>
  <div class="password w-100 bg-white">
    ...
    <nav-bar :title="title" mode="white" />
    ...
    <!-- <div class="order-content p-[22px] pt-0 bg-[#fff] flex items-center flex-wrap justify-center">
      ...
      <div class="text-[#0b75ff] text-[30px] mb-2">
        忘记支付密码？
      </div>
      <van-button size="large" color="#0b75ff">确认</van-button>
    </div> -->
    <div class="order-content p-[22px] bg-[#fff] flex items-center flex-wrap justify-center">
      <div v-for="(item, index) in fieldList" :key="item.key + index" class="input-content w-[96%]">
      ...
      <div class="text-[#0b75ff] text-[30px] mb-2" v-if="isSetPass && !isForgetPass" @click="forgetPass">
        忘记支付密码？
      </div>
      <div v-if="isForgetPass" class="w-[96%]">
        <div class="bg-[#ebf1f6] p-[18px] w-full flex flex-wrap">
          ...
          <img :src="forget_pass" alt="" class="w-[320px] h-[170px] ml-[8px]">
          <div class="text-[22px] leading-[40px] text-[#787f8c] mt-[20px]">
            .请一手持认证证件，一手持白纸拍摄照片上传，白纸上需要
            书写<span class="text-[red]">申请当天日期和申请办理业务内容（重置支付密码）</span><br />
            .证件上的信息不可进行修改和遮挡，必须看清证件号和姓名：<br />
            .图片支持jpg、jpeg、png格式，文件小于10M。
          </div>
        </div>
        <div class="bg-[#ebf1f6] p-[18px] w-full flex flex-wrap my-[20px]">
          <div class="text-[22px] leading-[40px] text-[#787f8c] mt-[20px]">
            <span class="text-[red]">1.为了保障您的资金安全，您的交易密码在24小时内暂停使用，您将不能进行付款，转账，卖币等操作，可以正常进行买币操作。</span><br />
            2.申请将于1个工作日内处理完毕，审核结果将以GDpay站内消息形式发送通知，请注意在【消息】内查收审核结果<br />
            3.请牢记您设置的新支付密码，审核通过后您将可以使用新支付密码进行资金操作
          </div>
        </div>
        <div class="mt-[30px] mb-[30px]">
          <van-radio-group v-model="readed" direction="horizontal">
            <van-radio name="1" class="text-[24px]">我已阅读并了解</van-radio>
          </van-radio-group>
        </div>
      </div>
      <van-button size="large" color="#0b75ff" v-if="!isSetPass" @click="setPayPass">确认</van-button>
      <van-button size="large" color="#0b75ff" v-if="isSetPass && !isForgetPass" @click="editPayPass">确认</van-button>
      <van-button size="large" color="#0b75ff" v-if="isSetPass && isForgetPass" @click="resetPayPass">确认</van-button>
...
import { userPayPassAddApi, userPayPassUpdateApi, userPayPassResetApi } from '@/apis/user'
...
const togglePasswordStatus = (index: number) => {
  fieldList[index].type =
    fieldList[index].type === 'password' ? 'text' : 'password'
}
...
const editPayPass = async () => {
  ...
  if (!regPayPass.test(getValue('o_pass'))) return showInfoToast(getItem('o_pass')?.formatError || '')
  if (!getValue('n_pass')) return showInfoToast(getItem('n_pass')?.require || '')
  if (!regPayPass.test(getValue('n_pass'))) return showInfoToast(getItem('n_pass')?.formatError || '')
  if (!getValue('c_pass')) return showInfoToast(getItem('c_pass')?.require || '')
  if (!regPayPass.test(getValue('c_pass'))) return showInfoToast(getItem('c_pass')?.formatError || '')
