<template>
  <div class="alipay w-100 bg-white">
    <nav-bar :title="bind_bank.coin_address ? '支付宝信息' : '添加支付宝'" mode="white" />
    ...
    <div class="order-content p-[0px] bg-[#fff] flex items-center flex-wrap justify-center">
      ...
      <van-field v-model="coin_address" label="支付宝帐号" placeholder="请输入您的支付宝帐号" label-align="top"
        v-if="!bind_bank.coin_address" />
      <van-field v-model="bind_bank.coin_address" label="支付宝帐号" placeholder="请输入您的支付宝帐号" label-align="top"
      ...
      <van-dialog v-model:show="isConfirmDialog" title="身份验证" confirmButtonText="提交" theme="round-button"
        className="dialog-bulletin" @confirm="() => !bind_bank.coin_address ? addAlipay() : deleteAlipay()">
        <i class="icon-close close-dark block" @click="handleConfirmDialog(false)"></i>
        <div>
          <van-field v-model="pay_pass" placeholder="请输入支付密码" type="password" readonly clickable @touchstart.stop="showKeyboard = true" @focus="showKeyboard = true" />
        </div>
      </van-dialog>
