<template>
  <div class="bank-card w-100 bg-white">
    <nav-bar
      :title="bind_bank.account_bankname ? '银行卡信息' : '添加银行卡'"
      mode="white"
    />
    ...
    <div class="order-content p-[0px] bg-[#fff] flex items-center flex-wrap justify-center">
      ...
      />
      <van-field
        v-model="bind_bank.account_bankname"
        label="银行名称"
        placeholder=""
        label-align="top"
        disabled
      ...
      />
      <div
        class="bg-[#ebf1f6] rounded-[7px] w-full mx-[30px] mb-[40px] py-[26px] px-[17px] mt-[20px]"
        v-if="!bind_bank.coin_address"
      >
...
console.log(
  config?.value.Web_Set_WebReg?.is_autobank_qztj,
  'config?.value.Web_Set_WebReg?.is_autobank_qztj'
)
