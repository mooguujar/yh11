    ...
    <div class="order-content p-[0px] bg-[#fff] flex items-center flex-wrap justify-center">
      ...
      >
        <div className="step-item">
          ...
          <div class="item-title text-[var(--van-button-primary-background)]">身份信息</div>
        </div>
        <div
          :className="`step-dot ${
            verified_status >= 1 && verified_status != 3
        ...
        <div className="step-item">
          <div
            class="item"
            :class="verified_status >= 1 && verified_status != 3 ? 'bg_primary' : ''"
          >
        ...
        <div
          ...
          :className="`step-dot ${
            verified_status === 1 && !VerifiedCheck
              ? '!text-[var(--van-button-primary-background)]'
              : ''
          }`"
        >
        ...
        >
          <div
            class="item"
            :class="verified_status == 1 && !VerifiedCheck ? 'bg_primary' : ''"
          >
      ...
      <div
        ...
        v-if="authStore?.userInfo?.verified_status == 2 || VerifiedCheck"
      >
        ...
        <div class="botton_dow">
          <p
            ...
            class="red_f mb-[10px] text-[26px] text-center"
          >
            高级认证后，日买币，卖币不限额
          </p>
          <van-button
            ...
            @click="
              ;(advancedVerified = true),
                (VerifiedCheck = false),
                $router.replace('/newVerify?advanced_verified=1')
            "
          >
            继续高级认证
          </van-button>
...
import { InWEB, requireImg } from '@/utils/tools'
...
const getadvanced_v = query?.advanced_v || false
const advancedVerified = ref(!!advanced)
const advanced_v = ref(!!getadvanced_v)
...
const verified_status = computed(() => {
  ...
  // eslint-disable-next-line vue/no-side-effects-in-computed-properties
  if (advanced_v.value) VerifiedCheck.value = false
  // console.log('VerifiedCheck.value111', VerifiedCheck.value)
  if (authStore?.userInfo?.verified_status == 4) router.replace('/verifyList')
...
function faceInit() {
  ...
  let face_image_mask = document.getElementById('face_image_mask') as HTMLDivElement
  // console.log('face_image_mask', face_image_mask)
  if (face_image_mask) face_image_mask.style.display = 'none'
  ...
  EsLivingDetection.verifyInit(
    ...
    .then((result: any) => {
      // if (face_image_mask) face_image_mask.style.display = 'block'
    ...
    .catch((ex: any) => {
      ...
      if (face_image_mask) face_image_mask.style.display = 'none'
      console.error(ex)
      // startDetect.value = false
      if (ex.code == 'ELD_UNSUPPORT') {
        ELD_UNSUPPORT.value = true
        errText.value = '当前浏览器无法打开摄像头,请换手机尝试'
        showInfoToast(errText.value)
        console.log(errText.value)
      } else if (ex.code == 'ELD_PERMISSION_DENIED') {
        ELD_UNSUPPORT.value = true
        errText.value = ex.msg || '当前浏览器无法获取到摄像头权限,请打开设备权限'
        showInfoToast(errText.value)
        console.log(errText.value)
      } else if (
        ex.code == 'ELD_UNSUPPORT' ||
        ex.code == 'ELD_NOTSUPPORT' ||
        ex.code == 'ELD_EXCEPTION' ||
        ex.code == 'ELD_TIMEOUT' ||
        ex.code == 'ELD_FAILED'
      ) {
        ELD_UNSUPPORT.value = true
        errText.value = ex.msg
        showInfoToast(errText.value)
        console.log(errText.value)
      } else {
        ELD_UNSUPPORT.value = true
...
:deep(#videoContainer) {
  ...
  #es_living_detection_main_container {
    ...
    #es_living_detection_canvasOutPut_container {
      ...
      #es_living_detection_mask1 {
        display: none;
      }
      #face_image_mask1 {
        display: none;
      }
      .es_living_detection_process_bar1 {
        display: none;
      }
    }
  }
